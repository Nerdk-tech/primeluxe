...
// 1. Re-run Table Creation (Ensure basic structure exists)
$tables = [
...
// 2. SMART COLUMN PATCHER (Removed "AFTER" to prevent the error you just got)
function addColumnSimple($conn, $table, $column, $definition) {
    $check = mysqli_query($conn, "SHOW COLUMNS FROM `$table` LIKE '$column'");
    if (mysqli_num_rows($check) == 0) {
        // We just add it to the end of the table so it never fails
...
// Fix Investments table (This is where your error was!)
