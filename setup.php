...
// 1. Core Table Creation (For fresh installations)
$tables = [
    ...
    "CREATE TABLE IF NOT EXISTS investments (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, amount_invested DECIMAL(10,2), status VARCHAR(20) DEFAULT 'active', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
];
...
function addColumnSimple($conn, $table, $column, $definition) {
    ...
    if (mysqli_num_rows($check) == 0) {
        ...
        if(mysqli_query($conn, $sql)) {
            echo "<p style='color:blue;'>Successfully added <b>$column</b> to <b>$table</b></p>";
        } else {
...
// Fix Investments (THE CRITICAL FIX)
...
echo "<p style='color:green; font-weight:bold;'>✅ All tables and columns are now 100% synchronized!</p>";
echo "<hr>";
